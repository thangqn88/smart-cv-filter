...
public class RoleService : IRoleService
{
    ...
    public async Task<List<string>> GetRolePermissionsAsync(string roleName)
    {
        try
        {
            var result = await _apiService.MakeRequestAsync<List<string>>($"roles/{Uri.EscapeDataString(roleName)}/permissions", HttpMethod.Get);
    ...
    public async Task<bool> UpdateRolePermissionsAsync(string roleName, List<string> permissions)
    {
        try
        {
            var result = await _apiService.MakeRequestAsync<object>($"roles/{Uri.EscapeDataString(roleName)}/permissions", HttpMethod.Put, permissions);
            return result != null;
        }
        catch (Exception ex)
        {
    ...
    public async Task<List<string>> GetAllPermissionsAsync()
    {
        try
        {
            var result = await _apiService.MakeRequestAsync<List<string>>("roles/permissions/all", HttpMethod.Get);
            return result ?? new List<string>();
        }
        catch (Exception ex)
        {
    ...
    public async Task<Dictionary<string, string>> GetPermissionDescriptionsAsync()
    {
        try
        {
            var result = await _apiService.MakeRequestAsync<Dictionary<string, string>>("roles/permissions/descriptions", HttpMethod.Get);
