...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.Console()
    .WriteTo.File("logs/smartcvfilter-.txt", rollingInterval: RollingInterval.Day)
    .CreateLogger();
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "Smart CV Filter API", Version = "v1" });
...
// HTTPS redirection removed - runs on HTTP only
app.UseStaticFiles();
app.UseRouting();
...
// Seed database (optional - skip if database is not available)
try
{
...
{
    Log.Warning(ex, "Database seeding skipped - database may not be available");
}
